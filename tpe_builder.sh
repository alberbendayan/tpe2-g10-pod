...
if [ "$CLIENT" = true ] ; then
  ...
  chmod +x tpe1-g10-client-1.0-SNAPSHOT/*.sh
  popd &>/dev/null

  [ -d bin ] || mkdir bin
  [ -d bin/client ] && rm -r bin/client
  mv client/target/tpe1-g10-client-1.0-SNAPSHOT bin/client
fi
...
if [ "$SERVER" = true ] ; then
  ...
  chmod +x tpe1-g10-server-1.0-SNAPSHOT/*.sh
  popd &>/dev/null

  [ -d bin ] || mkdir bin
  [ -d bin/server ] && rm -r bin/server
  mv server/target/tpe1-g10-server-1.0-SNAPSHOT bin/server
fi
